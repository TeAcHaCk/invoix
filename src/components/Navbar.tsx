...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="bg-slate-950/90 backdrop-blur-xl border-b border-slate-800/80 sticky top-0 z-40 px-3 sm:px-6 py-2 select-none font-['Plus_Jakarta_Sans',sans-serif]">
      <div className="max-w-[1800px] mx-auto flex items-center justify-between gap-3">
        {/* Left: Brand Identity, Mode & Live Auto-Save */}
        <div className="flex items-center space-x-2.5 shrink-0">
          {onNavigateToHome && (
            <button
              type="button"
              onClick={onNavigateToHome}
              className="p-1.5 rounded-xl glass text-slate-400 hover:text-amber-400 transition-colors flex items-center space-x-1 cursor-pointer"
              title="Return to Landing Page"
            >
              <Globe className="w-3.5 h-3.5" />
              <span className="text-[10px] font-semibold hidden lg:inline">Home</span>
            </button>
          )}

          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 p-0.5 shadow-md shadow-amber-500/20 flex items-center justify-center shrink-0">
            <div className="w-full h-full bg-slate-950 rounded-[9px] flex items-center justify-center text-sm">
              {preset.icon || '💼'}
            </div>
          </div>

          <div className="flex items-center space-x-2">
            <h1 className="text-xs sm:text-sm font-bold text-amber-100 tracking-wide font-['Outfit'] truncate max-w-[100px] sm:max-w-[160px]">
              {doc.studio.name || 'INVOIX'}
            </h1>
            ...
            <div className="flex items-center">
              {saveStatus === 'saving' ? (
                <div
                  className="flex items-center space-x-1 px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[10px] font-semibold animate-pulse"
                ...
                <div
                  className="flex items-center space-x-1 px-2 py-0.5 rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-300 text-[10px] font-semibold"
                  title="Auto-save failed. Click manual Save to retry."
                >
                ...
                <div
                  className="flex items-center space-x-1 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/25 text-emerald-300 text-[10px] font-medium"
                  title={
                    lastSavedTime
                ...
                >
                  ...
                  <span className="hidden sm:inline">{user ? 'Cloud' : 'Local'}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Center: 3-Way Mode Switcher (Split | Editor | Canvas) */}
        {onViewModeChange && (
          <div className="hidden md:flex items-center bg-slate-900/90 rounded-xl p-1 border border-slate-800 shadow-inner">
            <button
              type="button"
        ...
        <div className="flex items-center space-x-2 shrink-0">
          ...
          {onOpenHealth && (
            <button
              ...
              title="Inspect document readiness and verify layout"
            >
              {blockingCount > 0 ? (
          ...
          <div ref={shareRef} className="relative">
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 text-xs font-bold rounded-xl border border-emerald-500/30 transition-all cursor-pointer"
              title="Share proposal link or WhatsApp"
            >
          ...
          {/* Primary: Save PDF CTA (Gold Button) */}
          <div ref={pdfMenuRef} className="relative">
            <div className="inline-flex rounded-xl shadow-lg shadow-amber-500/20 bg-gradient-to-r from-amber-500 to-amber-600">
              <button
                ...
                className="flex items-center space-x-1.5 px-3.5 py-1.5 text-slate-950 text-xs font-extrabold hover:opacity-95 transition-all cursor-pointer disabled:opacity-50"
              ...
              <button
                ...
                className="px-1.5 py-1.5 text-slate-950 border-l border-amber-600/50 hover:bg-amber-400/40 rounded-r-xl transition-colors cursor-pointer"
              ...
              <div className="absolute right-0 top-full mt-2 w-56 glass rounded-2xl p-2 shadow-2xl shadow-black/40 border border-slate-700/50 z-50 modal-enter">
                ...
                {isAdmin && onOpenAdmin && (
                  <button
                    ...
                    className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs text-amber-300 hover:bg-slate-800/60 transition-colors cursor-pointer"
                  >
