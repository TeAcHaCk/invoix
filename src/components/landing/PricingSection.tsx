    ...
    >
      ...
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 reveal-on-scroll">
          {/* Currency */}
          <div className={`p-1.5 rounded-2xl flex items-center border ${isDark ? 'glass-dark border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
          ...
          <div className={`p-1.5 rounded-2xl flex items-center border ${isDark ? 'glass-dark border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
            ...
            >
              ...
              <span className={`text-[9px] px-2 py-0.5 rounded-full font-extrabold uppercase ${
                isAnnual ? 'bg-slate-950 text-amber-300' : 'bg-emerald-500/20 text-emerald-500'
              }`}>
                2 Months Free
              </span>
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          <div className={`reveal-on-scroll reveal-delay-1 rounded-3xl p-7 sm:p-8 space-y-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 border ${
            isDark ? 'glass-dark border-slate-800/80 hover-glow-blue' : 'glass-light border-slate-200 shadow-md shadow-slate-200/50'
          }`}>
            <div className="space-y-5">
              <div className="space-y-1">
                <h3 className={`text-lg font-bold font-['Outfit'] ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Starter Free</h3>
                <p className="text-xs text-slate-400">Great for getting started & exploring presets.</p>
              </div>

              <div className="py-2">
                <span className={`text-3xl font-extrabold font-mono ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{curr.symbol}0</span>
                <span className="text-xs text-slate-500"> / forever</span>
              </div>

              <ul className={`space-y-3 text-xs pt-4 border-t ${isDark ? 'text-slate-300 border-slate-800' : 'text-slate-600 border-slate-200'}`}>
                {[
            ...
            <button
              ...
              className={`w-full py-3.5 font-bold rounded-2xl text-xs transition-all cursor-pointer border ${
                isDark ? 'bg-slate-900 hover:bg-slate-800 text-slate-200 border-slate-700' : 'bg-slate-100 hover:bg-slate-200 text-slate-900 border-slate-300'
              }`}
            >
              Get Started Free
            </button>
          ...
          <div className="reveal-on-scroll reveal-delay-2 relative">
            <div className="absolute -inset-[2px] bg-gradient-to-br from-amber-400 via-orange-500 to-amber-400 rounded-[26px] opacity-70 blur-[1px]" />
            <div className={`relative rounded-3xl p-7 sm:p-8 space-y-6 flex flex-col justify-between h-full ${
              isDark ? 'bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 text-slate-100' : 'bg-white text-slate-900 shadow-2xl'
            }`}>
              {/* Most Popular Badge */}
              ...
              <div className="space-y-5 pt-2">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400">For freelancers, studio owners & active closers.</p>
                </div>

                <div className="py-2">
                  <span className={`text-4xl font-extrabold font-mono ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                    {curr.symbol}
                ...
                <ul className={`space-y-3 text-xs pt-4 border-t ${isDark ? 'text-slate-200 border-slate-800' : 'text-slate-700 border-slate-200'}`}>
                  {[
          ...
          <div className={`reveal-on-scroll reveal-delay-3 rounded-3xl p-7 sm:p-8 space-y-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 border ${
            isDark ? 'glass-dark border-slate-800/80 hover-glow-purple' : 'glass-light border-slate-200 shadow-md shadow-slate-200/50'
          }`}>
            <div className="space-y-5">
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <h3 className={`text-lg font-bold font-['Outfit'] ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Agency Growth</h3>
                  <Crown className="w-4 h-4 text-purple-400" />
                </div>
                <p className="text-xs text-slate-400">For multi-person teams, consultancies & agencies.</p>
              </div>
